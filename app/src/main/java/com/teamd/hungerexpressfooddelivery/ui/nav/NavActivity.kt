...
class NavActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(isAdmin) {
            // Show only the options relevant to admin
            binding.bottomNavigationView.menu.findItem(R.id.restaurantFragment)?.isVisible = true
            // Hide the "My orders" and "cart" buttons for admin
            binding.bottomNavigationView.menu.findItem(R.id.ordersFragment)?.isVisible = false
            binding.bottomNavigationView.menu.findItem(R.id.cartFragment)?.isVisible = false
        } else {
            // Show the options relevant to regular users
